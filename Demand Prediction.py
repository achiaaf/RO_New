...
from Functions import AGMC_13, projection, AGMC_12a, grey_degree2, MAPE, linear_model, linear_param, objective_function
...
folder_path = 'C:/Users/User/Documents/RO_New/Demand Prediction'
...
# opt_param = [0.009, 0.0001, 0.0002, 0.01]
# opt_param = []
# # Finding the optimized parameter
# for i in range(1, areas+1):
#     X = np.array(water_consumption.iloc[:6, i])
#     Y = np.array(gdp.iloc[a:6, 1])
#     Z = np.array(population.iloc[:6, i])
#     w_bounds = ([0], [1.0])
#     # Finding the optimized parameter
#     optimized_w, _ = pso(objective_function, w_bounds[0], w_bounds[1], args=(X, Y, Z), maxiter=1000)
#     opt_param.append(optimized_w)
# X = np.array(water_consumption.iloc[2:, 3])
# Y = np.array(gdp.iloc[2:, 1])
# Z = np.array(population.iloc[2:, 3])
# w_bounds = ([-1.0], [1.0])
# # Finding the optimized parameter
# optimized_w, _ = pso(objective_function, w_bounds[0], w_bounds[1], args=(X, Y, Z), maxiter=1000)
# print(optimized_w)
...
opt_param = [np.array([0.267]), np.array([0.119]), np.array([0.086]), np.array([0.17])]
error = pd.DataFrame({'Model': ['Regression Model', 'AGMC_13']})#, 'AGMC_13']})#, 'ANN']})
b = [6, 8, 7, 8]
for i in range(1, areas+1):
    water_consumption_train = np.array(water_consumption.iloc[a:b[i-1], i])
    ...
    # predict_water1 = (np.array(AGMC_12a(water_consumption_train, pop_new, opt_param[i-1])))/1000
    p1 = linear_param(water_consumption.iloc[a:7, i], gdp.iloc[a:7, 1], population.iloc[a:7, i])
    predict_water2 = linear_model(gdp_new, pop_new, p1)
    predict_water4 = (np.array(AGMC_13(water_consumption_train, pop_new, gdp_new, opt_param[i-1])))
    ...
    d1.insert(loc=len(d1.columns), column=f'AGMC13 Area {i}', value=predict_water4)
    ...
    mape2 = MAPE(water_consumption.iloc[:, i], predict_water2[0:len(water_consumption.iloc[:, i])])
    # # mape3 = MAPE(water_consumption.iloc[:, i]/1000, predict_water3[0:len(water_consumption.iloc[:, i])])
    mape4 = MAPE(water_consumption.iloc[:, i], predict_water4[0:len(water_consumption.iloc[:, i])])
    # mse1 = mean_squared_error(water_consumption.iloc[:, i]/1000, predict_water1[0:len(water_consumption.iloc[:, i])])
    mse2 = mean_squared_error(water_consumption.iloc[:, i], predict_water2[0:len(water_consumption.iloc[:, i])])
    mse4 = mean_squared_error(water_consumption.iloc[:, i], predict_water4[0:len(water_consumption.iloc[:, i])])
    ...
    # plt.plot(year_new[0:len(water_consumption.iloc[a:, i])], predict_water4[0:len(water_consumption.iloc[a:, i])]/1000, label='Modelling Values by AGMC(1,N) Model', color='g', marker='^', linewidth=2)
    # plt.plot(year_new[0:len(water_consumption.iloc[a:, i])], predict_water2[0:len(water_consumption.iloc[a:, i])]/1000, label='Modelling Values by MLR Model', color='r', marker='o', linewidth=2)
    ...
    # plt.ylabel('Water Consumption (x10$^3$ m$^3$)', fontweight='bold')
    # plt.title(f'Area {i}', fontweight='bold')
    # plt.xticks()
    # plt.yticks()
    # plt.legend()
    # plt.show()
    ...
    plt.figure()
    # plt.rcParams['font.size'] = '16'
    # plt.plot(year_new[len(water_consumption.iloc[a:, i]):], water_consumption.iloc[a:, i]/1000, label='Actual Water Consumption', color='c', marker='*', linewidth=2)
    plt.plot(year_new[len(water_consumption.iloc[a:, i]):], predict_water4[len(water_consumption.iloc[a:, i]):]/1000, label='Modelling Values by AGMC(1,N) Model', color='g', marker='^', linewidth=2)
    plt.plot(year_new[len(water_consumption.iloc[a:, i]):], predict_water2[len(water_consumption.iloc[a:, i]):]/1000, label='Modelling Values by MLR Model', color='r', marker='o', linewidth=2)
    # plt.plot(year_new[0:len(water_consumption.iloc[a:, i])], predict_water4[0:len(water_consumption_train)], label='Modelling Values by AGMC13 Model', color='b')
    # plt.plot(year_new[0:len(water_consumption.iloc[a:, i])], predict_water3[0:len(water_consumption.iloc[a:, i])], label='Modelling Values by ANN Model', color='m', marker='s')
    plt.xlabel('Year', fontweight='bold')
    plt.ylabel('Water Consumption (x10$^3$ m$^3$)', fontweight='bold')
    ...
    # plt.show()
# error.to_excel(os.path.join(folder_path, 'Error for Demand Prediction.xlsx'))
# d1.to_excel(os.path.join(folder_path, 'Demand Prediction Results_final.xlsx'))
