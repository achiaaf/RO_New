...
for i in range(1, areas+1):
    ...
    error.append(math.sqrt(mape/100))
...
def rec_unc(ohm):
    ...
    for a in range(areas):
        ...
        #     model.st((qSc_vars[a].sum(axis=0)[z] + qDc_vars[a].sum(axis=0)[z] + qS_vars[a][z] + qD_vars[a][z] <= 0.9 *
        #               (cum_prod[a, z])
        #               + cum_supply[a][z] + (delta[a][:, z] @ r_uncertain[a]) - cum_supply[a][z]).forall(r_set[a]))
    ...
    for a in range(areas):
        ...
        for z in range(time):
            model.st((qSc_vars[a].sum(axis=0)[z] + qDc_vars[a].sum(axis=0)[z] + qS_vars[a][z] + qD_vars[a][z] <= 0.9 *
                      (cum_prod[a, z])
                      + cum_recharge[a, z] + (delta[a][:, z] @ r_uncertain[a]) - cum_supply[a][z]).forall(r_set[a]))
            # Change  probably the line above by chqnging the recharge factor
