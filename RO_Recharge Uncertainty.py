...
folder_path = f'C:/Users/User/OneDrive - BGU/Documents/Life at BGU/Research Work/PycharmProjects/pythonProject/venv/' \
              f'Scripts/RO_New/Recharge Uncertainty Results'
...
for v in range(4):
    ...
    for i in range(areas):
        ...
        model.st(qS_vars[i] + qD_vars[i] >= water_demand[i])
        ...
        # Sources Constraint
        model.st(qW_vars[i].sum(axis=0) <= 0.6 * water_demand[i])
        ...
        for n in range(time):
            model.st((qSc_vars[i].sum(axis=0)[n] + qDc_vars[i].sum(axis=0)[n] + qS_vars[i][n] + qD_vars[i][n] <= 0.9 *
                      (quantity[i])
                      + recharge_cum[i][n] + (delta[i, i] * r_uncertain[i][n]) - cum_supply[i][n]).forall(r_set[i]))
        # Land Constraint
    ...
    #     (pd.concat([pd.concat([d5, d6], axis=1), d7], axis=1)).to_excel(os.path.join(folder_path, f'Domestic Use for Area {i + 1} Ω{radius[v]}.xlsx'))
    #     (pd.concat([h_aquifer, d1, h_tww, d2, h_desal, d3, h_land, d4], axis=0)).to_excel(os.path.join(folder_path,
    #                                                                                                    f'Crops Output for Area {i + 1} Ω{radius[v]}.xlsx'))
...
# df.to_excel(os.path.join(folder_path, 'Optimal Solutions.xlsx'))
