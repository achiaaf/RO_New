...
def prob_vio(q1, q2, q3, q4, rs):
    ...
    for p in range(1000):
        ...
        prob.append(np.sum(proba) > len(proba))   # This gives a sum of all falses.
        # print(proba)
...
def prob_vio_demrec(q1, q2, q3, q4, q5,  rs):
    ...
    for r in range(areas):
        dem_vec = []
        for w in range(time):
            dem_vec.append(np.random.normal(water_demand[r][w], error[r] * water_demand[r][w], 1000))
    ...
    for p in range(1000):
        ...
        # print(proba1)
        proba2 = [~arr.astype(bool) for arr in proba1]
        prob.append(np.sum(proba) + np.sum(proba1) < 240)   # This gives a sum of all falses.
